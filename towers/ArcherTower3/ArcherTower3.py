...
class ArcherTower3(pygame.sprite.Sprite):
    def __init__(self, win):
        ...
        self.images_archers = []
        for i in range(1, 7):
            self.images_archers.append(pygame.transform.scale(pygame.image.load('towers/ArcherTower3/archer_1_' + str(i) + '.png'), (45, 45)))
        self.images_tower = [pygame.transform.scale(pygame.image.load('towers/ArcherTower3/tower_1_2.png'), (120, 115))]
        self.img_count = 0
        self.count = 0
        self.power = 100
    ...
    def draw(self):
        ...
        if self.target:
            ...
            if self.x > self.target.x:
                self.win.blit(pygame.transform.flip(self.images_archers[self.img_count], 1, 0),
                              (self.x + 30, self.y - 25))
            else:
        ...
        self.win.blit(self.arrow, (self.arrow_pos[0], self.arrow_pos[1]))
    ...
    def is_hit(self):
        if self.arrow_pos[0] + 32 > self.target.x > self.arrow_pos[0] - 32 and self.arrow_pos[
            1] + 32 > self.target.y > \
                self.arrow_pos[1] - 32:
            ...
            range = math.sqrt(
                math.pow((self.arrow_pos[0] - self.target.x), 2) + math.pow((self.arrow_pos[1] - self.target.y), 2))
            if self.range < range:
                self.target = None
                self.arrow_pos = [-100, -100]
